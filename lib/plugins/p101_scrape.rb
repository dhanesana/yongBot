  ...
  module Plugins
    class P101
      ...
      def with_num(m, prefix, p101, rank)
        ...
        href = page.css('li.mn_04 dd').last.children.first.attributes['href'].value
        recent_page = Nokogiri::HTML(open("http://mnettv.interest.me#{href}"))
        name = recent_page.css('p.name')[rank.to_i - 1].text
        agency = recent_page.css('p.agency')[rank.to_i - 1].text
        votes = recent_page.css('p.voteCount')[rank.to_i - 1].text
        href = recent_page.css('div.thumb a')[rank.to_i - 1].first[1]
      ...
      def with_name(m, prefix, p101, name)
        ...
        recent_page = Nokogiri::HTML(open("http://mnettv.interest.me#{href}"))
        count = 0
        recent_page.css('p.name').each do |contestant|
