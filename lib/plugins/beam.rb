...
module Cinch
  module Plugins
    class Beam
      ...
      match /(beamban) (.+)/, method: :ban_unban
      match /(beamban) (list)$/, method: :ban_list
      ...
      def banned_table(conn)
        # banned users db
        begin
          res = conn.exec_params("CREATE TABLE beambanned (prefix varchar);")
          conn.exec("INSERT INTO beambanned (prefix) VALUES ('user/testban');")
        rescue PG::Error => pg_error
          puts '*' * 50
          puts "Table creation failed: #{pg_error.message}"
        end
        banned_users = conn.exec("SELECT * FROM beambanned;")
        @banned = []
      ...
      def ban_toggle(m, user_prefix)
        conn = PG::Connection.new(ENV['DATABASE_URL'])
        search = conn.exec("SELECT * FROM beambanned WHERE prefix='#{conn.escape_string(user_prefix)}';")
        if search.ntuples > 0
          conn.exec("DELETE FROM beambanned WHERE prefix='#{conn.escape_string(user_prefix)}';")
          return User(m.user.nick).notice("#{conn.escape_string(user_prefix)} is UNbanned from adding beam users")
        else
          conn.exec("INSERT INTO beambanned (prefix) VALUES ('#{conn.escape_string(user_prefix)}');")
          return User(m.user.nick).notice("#{conn.escape_string(user_prefix)} is banned from adding beam users")
        end
        banned_users = conn.exec("SELECT * FROM beambanned;")
        @banned = []
        banned_users.each do |row|
      ...
      def ban_list(m)
        ...
        string = ""
        if m.is_admin?
          get_all = conn.exec("SELECT * FROM beambanned;")
          get_all.each do |x|
            string += "#{x['prefix']}"
            string += "\n"
          end
        ...
        # Unlisted paste titled '.beambanned list' expires in 10 minutes
        m.user.msg(pastebin.newpaste(string, api_paste_name: '.beamban list', api_paste_private: 1, api_paste_expire_date: '10M'))
        m.reply "check ur pms for list of beambanned prefixes"
      end
